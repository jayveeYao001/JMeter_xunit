    ...
    <xsl:template match="/testResults">
        ...
        <head>
            ...
            <script type="text/javascript"><![CDATA[
                ...
                //
                function show(this_testFlag, flag) {
                    console.log('flag=' +flag +'    this=');
                    console.log(this_testFlag);
                    var eleLs= document.getElementsByName(flag);
                    for (var i=0; i<eleLs.length; i++) {
                        if (this_testFlag.textContent == eleLs[i].firstChild.firstChild.textContent) {
                            ele = eleLs[i];
                            break;
                        }
                    }
        ...
        <body>
            <div id="left-panel">
                <!-- <ol id="result-list"> -->
                <xsl:for-each select="sample[@lb='@TM']">
                    ...
                    -->
                    <li name="TestModule" onclick="show(this,'tmSw')">
                        <xsl:attribute name="class">
                            <xsl:choose>
                                <xsl:when test="@ATRes = 'true'">success</xsl:when>
                                <xsl:otherwise>failure</xsl:otherwise>
                            </xsl:choose>
                        </xsl:attribute>
                        <xsl:value-of select="child::sample[1]/@lb"/>
                    </li>
                    <div name="tmSw" style="display:none">
                    <!-- -->
                    <div class="success"><xsl:value-of select="child::sample[1]/@lb"/></div>
                    <!-- -->
                    <xsl:for-each select="sample[@lb='@TI' or @lb='@TI_Special']">
                        <li name="TestItem" onclick="show(this,'tiSw')">
                            <xsl:attribute name="class">
                                <xsl:choose>
                                    <xsl:when test="@ATRes = 'true'">success</xsl:when>
                                    <xsl:otherwise>failure</xsl:otherwise>
                        ...
                        <div name="tiSw" style="display:none">
                                ...
                                <div>
                                    ...
                                    <div class="zebra">
                                        <table>
                                            ...
                                            <tr><td class="data key">Sampler Datas</td><td class="data delimiter">:</td><td class="data"><pre class="data"><xsl:value-of select="samplerData"/></pre></td></tr>
                                        </table>
                                    </div>
                                    <div class="trail"></div>
                                    ...
                                    <div class="trail"></div>
                                    <div class="group">SubSampler</div>
                                    <div class="zebra">
                                        <table>
                                            <tr><td class="data key">SubSampler RespData</td><td class="data delimiter">:</td><td class="data"><pre class="data"><xsl:value-of select="sample/responseData"/></pre></td></tr>
                                        </table>
                                    </div>
                                    <div class="trail"></div>
                                </div>
